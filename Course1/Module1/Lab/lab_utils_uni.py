...
def plt_divergence(p_hist, J_hist, x_train,y_train):
    ...
    for i in range(len(w_array)):
        tmp_w = w_array[i]
        print(f"tmp_w: {tmp_w}, cost: {compute_cost(x_train, y_train, tmp_w, fix_b)}")
        cost[i] = compute_cost(x_train, y_train, tmp_w, fix_b) / 1e6  # scale to millions
    ...
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j]) / 1e6
